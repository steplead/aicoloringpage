...
export async function generateImage(prompt: string, style: string = 'kawaii') {
    ...
    // User requested "2.5 flash" which corresponds to the experimental 2.0 Flash model
  ...
  8. Do NOT output text, only the image.`;
    ...
    try {
        ...
        // Handle Text response (Error case for Image Gen)
            ...
            // Throw a specific error so we know what happened
            throw new Error('AI returned text instead of image. Please try again or adjust prompt.');
        } else {
